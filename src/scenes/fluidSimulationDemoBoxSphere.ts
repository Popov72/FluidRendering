...
export class FluidSimulationDemoBoxSphere extends FluidSimulationDemoBase {
    ...
    constructor(scene: BABYLON.Scene) {
        ...
        this._spherePos = new BABYLON.Vector3(
            (this._boxMin.x + this._boxMax.x) / 2,
            this._boxMin.y + this._sphereRadius,
        ...
        this._collisionPlanesFloorOnly = [this._origCollisionPlanes[5].clone()];
        this._sphereCollisionRestitution = 0.98;
    ...
    public async run() {
        ...
        // Create materials
        this._sphereMaterial = new BABYLON.PBRMaterial(
            "collisionMeshMat",
            this._scene
        );
        this._sphereMaterial.metallic = 1;
        this._sphereMaterial.roughness = 0.05;
        this._sphereMaterial.albedoTexture = new BABYLON.Texture(
            marbleBaseColor,
            this._scene
        );
        ...
        // Create meshes
        this._sphereMesh = BABYLON.MeshBuilder.CreateSphere(
            "collisionMesh",
            { diameter: this._sphereRadius * 2, segments: 16 },
            this._scene
        );
    ...
    protected _makeGUIMainMenu(): void {
        ...
        mainMenu
            .add(params, "sphereCollisionRestitution", 0, 1, 0.01)
        ...
        mainMenu
            .add(params, "boxCollisionRestitution", 0, 1, 0.01)
    ...
    protected _rotateMeshes(angleX: number, angleY: number): void {
        ...
        for (let i = 0; i < this._origCollisionPlanes.length; ++i) {
            this._collisionPlanes[i] =
                this._origCollisionPlanes[i].transform(transfo);
        }
    ...
    protected _checkCollisions(particleRadius: number): void {
        ...
        const collisionsPlanes = this._checkXZBounds
            ? this._collisionPlanes
        ...
        for (let a = 0; a < this._fluidSim!.currentNumParticles; ++a) {
            ...
            if (d < sr * sr) {
                ...
                velocities[a * 3 + 0] =
                    (velocities[a * 3 + 0] - 2 * dotvn * nx) *
                    this._sphereCollisionRestitution;
                velocities[a * 3 + 1] =
                    (velocities[a * 3 + 1] - 2 * dotvn * ny) *
                    this._sphereCollisionRestitution;
                velocities[a * 3 + 2] =
                    (velocities[a * 3 + 2] - 2 * dotvn * nz) *
                    this._sphereCollisionRestitution;
            ...
            for (let i = 0; i < collisionsPlanes.length; ++i) {
                ...
                const dist =
                    plane.normal.x * positions[a * 3 + 0] +
                    plane.normal.y * positions[a * 3 + 1] +
                    plane.normal.z * positions[a * 3 + 2] +
                    plane.d -
                    particleRadius;
                if (dist < 0) {
                    const dotvn =
                        velocities[a * 3 + 0] * plane.normal.x +
                        velocities[a * 3 + 1] * plane.normal.y +
                        velocities[a * 3 + 2] * plane.normal.z;

                    velocities[a * 3 + 0] =
                        (velocities[a * 3 + 0] - 2 * dotvn * plane.normal.x) *
                        this._boxCollisionRestitution;
                    velocities[a * 3 + 1] =
                        (velocities[a * 3 + 1] - 2 * dotvn * plane.normal.y) *
                        this._boxCollisionRestitution;
                    velocities[a * 3 + 2] =
                        (velocities[a * 3 + 2] - 2 * dotvn * plane.normal.z) *
                        this._boxCollisionRestitution;
